<?php
// admin/edit_pendapatan.php - UPDATE pendapatan
...
$id = (int)($_GET['id'] ?? 0);
$stmt = mysqli_prepare($koneksi, 'SELECT * FROM pendapatan WHERE id = ?');
mysqli_stmt_bind_param($stmt, 'i', $id);
mysqli_stmt_execute($stmt);
$r = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));
mysqli_stmt_close($stmt);
if (!$r) { set_flash('error', 'Transaksi tidak ditemukan.'); header('Location: pendapatan.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = mysqli_prepare($koneksi, 'UPDATE pendapatan SET kos_id=?, kategori_id=?, tanggal=?, jumlah=?, keterangan=? WHERE id=?');
        mysqli_stmt_bind_param($stmt, 'iisisi', $r['kos_id'], $r['kategori_id'], $r['tanggal'], $r['jumlah'], $r['keterangan'], $id);
        $ok = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        if ($ok) { set_flash('success', 'Pendapatan diperbarui.'); header('Location: pendapatan.php'); exit; }
        $errors[] = 'Gagal update.';
    }
}

render_header('Edit Pendapatan', 'pendapatan');
?>
<div class="page-header"><h4 class="mb-0"><i class="bi bi-pencil"></i> Edit Pendapatan</h4></div>
