<?php
// admin/pengeluaran.php - daftar pengeluaran (JOIN kos+kategori, filter bulan/tahun)
...
$stmt = mysqli_prepare($koneksi, "SELECT p.*, k.nama_kos, kat.nama AS kategori
    FROM pengeluaran p JOIN kos k ON p.kos_id=k.id JOIN kategori kat ON p.kategori_id=kat.id"
...
render_header('Pengeluaran', 'pengeluaran');
show_flash();
?>
<div class="page-header">
    <h4 class="mb-0"><i class="bi bi-arrow-up-circle"></i> Pengeluaran</h4>
    <a href="tambah_pengeluaran.php" class="btn btn-primary btn-sm ms-auto"><i class="bi bi-plus-lg"></i> Tambah Pengeluaran</a>
</div>
...
<form method="get" class="row g-2 align-items-end mb-3">
    ...
    <div class="col-auto">
        <a href="pengeluaran.php" class="btn btn-sm btn-outline-secondary">Reset</a>
    </div>
</form>
    ...
    <table class="table table-striped table-hover align-middle mb-0">
        ...
        <?php else: foreach ($rows as $r): ?>
            <tr>
                <td><?= tgl($r['tanggal']) ?></td>
                <td><?= e($r['nama_kos']) ?></td>
                <td><span class="badge bg-danger"><?= e($r['kategori']) ?></span></td>
                <td><?= e($r['keterangan'] ?: '-') ?></td>
                <td class="text-end fw-bold text-danger"><?= rp((int)$r['jumlah']) ?></td>
                <td class="text-center text-nowrap">
                    <a href="edit_pengeluaran.php?id=<?= (int)$r['id'] ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                    <a href="hapus_pengeluaran.php?id=<?= (int)$r['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus transaksi ini?')"><i class="bi bi-trash"></i></a>
                </td>
            </tr>
        <?php endforeach; ?>
            <tr class="table-light">
                <td colspan="4" class="fw-bold">Total <?= date('F Y', mktime(0,0,0,$bln,1,$thn)) ?></td>
                <td class="text-end fw-bold fs-6 text-danger"><?= rp($total) ?></td>
                <td></td>
            </tr>
        <?php endif; ?>
        </tbody>
