<?php
// admin/tambah_pendapatan.php - CREATE pendapatan (dropdown kos + kategori filter Pendapatan, BR-8)
...
$kats = mysqli_fetch_all(mysqli_query($koneksi, "SELECT id, nama FROM kategori WHERE tipe='Pendapatan' ORDER BY nama"), MYSQLI_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = mysqli_prepare($koneksi, 'INSERT INTO pendapatan (kos_id, kategori_id, tanggal, jumlah, keterangan) VALUES (?,?,?,?,?)');
        mysqli_stmt_bind_param($stmt, 'iisis', $old['kos_id'], $old['kategori_id'], $old['tanggal'], $old['jumlah'], $old['keterangan']);
        $ok = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        if ($ok) { set_flash('success', 'Pendapatan ditambahkan.'); header('Location: pendapatan.php'); exit; }
        $errors[] = 'Gagal menyimpan.';
    }
}

render_header('Tambah Pendapatan', 'pendapatan');
?>
<div class="page-header"><h4 class="mb-0"><i class="bi bi-plus-lg"></i> Tambah Pendapatan</h4></div>
<a href="pendapatan.php" class="btn btn-link text-decoration-none ps-0 mb-3"><i class="bi bi-arrow-left"></i> Kembali</a>

<div class="card shadow-sm"><div class="card-body">
<?php if (!$koss): ?><div class="alert alert-warning">Belum ada kos. <a href="tambah_kos.php">Tambah kos dulu</a>.</div><?php endif; ?>
<?php if (!$kats): ?><div class="alert alert-warning">Belum ada kategori Pendapatan. <a href="tambah_kategori.php">Tambah kategori dulu</a>.</div><?php endif; ?>
...
<form method="post" novalidate>
    <div class="row g-3">
        ...
        <div class="col-12">
            ...
            <a href="pendapatan.php" class="btn btn-link text-decoration-none">Batal</a>
        </div>
    </div>
</form></div></div>
<?php render_footer(); ?>
